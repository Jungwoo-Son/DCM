...
describe('spec of Access Token', () => {
    ...
    describe('make access token from stringed token', () => {
        it('should make successfully', () => {
            const access_token = AccessToken.fromString(sample_stringed_access_token);
        ...
        it('should not make', () => {
            expect(() => 
                AccessToken.fromString(sample_stringed_refresh_token)
            ).toThrow();
        });
    });
});
...
describe('spec of Refresh Token', () => {
    it('should make valid AccessToken with user_id', () => {
        const refresh_token = new RefreshToken('asdf');
    ...
    describe('make refresh token from stringed token', () => {
        it('should make successfully', () => {
            const refresh_token = RefreshToken.fromString(sample_stringed_refresh_token);
            const expected_token = new RefreshToken('asdf');

            expect(refresh_token).toEqual(expected_token);
        });
        it('should not make', () => {
            expect(() => 
                RefreshToken.fromString(sample_stringed_access_token)
            ).toThrow();
        });
    });
});